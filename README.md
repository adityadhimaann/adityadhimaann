<div align="center">

Aditya Kumar - Full Stack and AI Developer

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat&logo=linkedin)](https://linkedin.com/in/adityadhimaann)
[![Portfolio](https://img.shields.io/badge/Portfolio-adidev.works-FF6B6B?style=flat&logo=firefox)](https://www.adidev.works)
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=flat&logo=gmail)](mailto:dhimanaditya56@gmail.com)
[![LeetCode](https://img.shields.io/badge/LeetCode-300%2B_Solved-orange?style=flat&logo=leetcode)](https://leetcode.com/AadiHamza)

</div>

---

```yaml
education:  B.Tech CSE @ Lovely Professional University  (2023–2027)
location:   Punjab, India
status:     Freelancing on Upwork + Building AI products 🔥
```

---

## ⚡ Stack

<p align="center">
<img src="https://skillicons.dev/icons?i=java,js,ts,cpp,react,nextjs,nodejs,express,mongodb,postgres,mysql,docker,git,gcp&theme=dark" />
</p>

`Gen AI` · `LLM Integration` · `Prompt Engineering` · `WebRTC` · `Socket.IO` · `REST APIs` · `Microservices`

---

## 🚀 Projects

| Project | What it does | Tech |
|--------|-------------|------|
| **[SyncRide](https://github.com/adityadhimaann)** | GenAI travel assistant · real-time delay prediction · door-to-door planning | TypeScript, Firebase, OpenAI |
| **[UniEd](https://github.com/adityadhimaann)** | EdTech platform · live classes via WebRTC · AI interview simulator | React, Node.js, MongoDB, GCP |

---

## 💼 Experience

**Freelancer @ Upwork** *(Dec 2025 – Present)*  
5+ full-stack apps delivered · 100% client satisfaction · AI-powered dashboards with LLM APIs · REST API response time improved by **35%** across SaaS & EdTech platforms.

**Web App Dev Intern @ Larsen & Toubro** *(Jun – Jul 2025)*  
Built *Campus Gateway* for 500+ employees · optimized MySQL queries cutting latency from 3.2s → 1.1s (**65% faster**) · deployed on IIS with Hyper-V.

---

## 🏆 Highlights

- 🥇 **Top 15 / 25,000+** — Finarva AI Hackathon (Gromo × AWS × Sarvam AI)
- 🇮🇳 **Smart India Hackathon 2025** — Selected for Govt. of Punjab idea presentation
- 💻 **300+ DSA problems** solved on LeetCode
- ☁️ Oracle Cloud Infrastructure 2025 · Google Generative AI · MongoDB Atlas certified

---
